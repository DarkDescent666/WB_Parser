...
class Page_Source(UserData,Writers):
    ...
    async def get_page_data(self,products,min_price=0, max_price=1000000000000000, rt=1):
        try:
            ...
            if self.file_writer == "JSON":
                await self.write_method_json(products_list,data_path=self.path)
            else:
                await self.write_method_csv(products_list,data_path=self.path)


        except:
            Exception(TypeError)
    async def gather_data(self,min_price=0, max_price=1000000000000000, rt=1):
